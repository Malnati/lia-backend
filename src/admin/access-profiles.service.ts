...
export class AccessProfilesService {
  ...
  async create(auth: AuthContext, dto: CreateAccessProfileDto): Promise<AccessProfileView> {
    const { data, error } = await this.supabase
      .getClient()
      .from('access_profiles')
      .insert({
  ...
  async update(auth: AuthContext, id: string, dto: UpdateAccessProfileDto): Promise<AccessProfileView> {
...
function mapAccessProfile(row: AccessProfileRow): AccessProfileView {
  return {
    id: row.id,
